...
from graniteledger.policy.allowance_annual import ConfigError, RGGIPolicyAnnual
...
def _ensure_pandas() -> None:
    ...
    if pd is None:  # pragma: no cover - helper exercised indirectly
        raise ImportError(
            "pandas is required for graniteledger.config.policy_loader; install it with `pip install pandas`."
        )
