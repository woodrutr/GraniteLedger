""" This script houses the model class, a set of methods and procedures built for 
constructing Pyomo models.

Model is meant to be inherited by other modules built within this architecture, as
the integration procedures being developed require consistent tracking of index values and
labels for all Pyomo components. See the class definition for more details.
"""

import pandas as pd
import numpy as np
from typing import Literal, List, Iterable, Tuple, Sequence, MutableSequence, Dict, DefaultDict, Set
...
class Model(pyo.ConcreteModel):
    ...
    Modules built within this platform should inherit the model class, as seen in LowRes and 
    HighRes (found in src/models).
    """
...
#        self.dim_dict = {}
    ...
    def build(
        ...
        This will work for any class inheriting the method, but it is recommended to replace this with model-specific
        build instructions if this functionality is desired.
    ...
    def reorganize_index_set(
        ...
        """Creates new pyomo sets based on an input set and a desired set of indices for an output set. User should provide 
        either names of columns desired for reorganized output set OR the name of a set that mirrors the desired indexing.
        ...
        If only the set keys are desired, without creating an indexed set object as illustrated above, the user can 
        set 'create_indexed_set' to false. If true, the output is a pyo.IndexedSet, with each element of the IndexedSet 
        containing the values of the other indices
        ...
        if reorg_set_sname:
            reorg_set = getattr(self, reorg_set_sname) # placeholder for a function that crosschecks the indices if desired
        ...
        if bool(missing_elements):
            raise ValueError(f"Elements missing from input set desired in new set: {missing_elements}")

        set_out_key_index = set([set_in_cols.index(x) for x in reorg_set_cols])
        set_out_val_index = set([set_in_cols.index(x) for x in set_in_cols if x not in reorg_set_cols])
    ...
    def declare_set(
        ...
        if isinstance(sdata, (pd.DataFrame, pd.Series)):
        ...
        elif isinstance(sdata, (list, tuple, range, set, np.ndarray)):
            ...
            if scols is None:
                raise ValueError("Provided dictionary data without index labels; specify labels in scols")
    ...
    def _declare_set_with_pandas(
        ...
        Function assumes that the index values are the desired data to construct set object. User can specify 
        whether to create set with column values instead
    ...
    def _declare_set_with_iterable(
        ...
        Function can take iterable objects such as tuples, lists, etc as data inputs. Note that if the dimension of the index is larger
        than 1, user needs to provide a list of names for each set dimension.
        ...
        pyo.Set
            Pyomo Set Object
        """
        # declare_set_iterable -- If switch off, return empty set
        if not switch:
            return None
        ...
        if len(sdata) == 0:
            empty = True
            scols = sname
            warnings.warn(f"For {sname}, sdata: {type(sdata)} is length 0; returning empty set called {sname}")
        else:
            empty = False
        ...
        if not scols and not empty:
            if isinstance(sdata[0], (list, tuple)) and len(sdata[0]) > 1:
                raise ValueError("if using list or set w/ multiple dimensional index, need to provide labels of indices")
            else:
                print(f"declare_set_with_iterable assumes desired index name is sname = {sname} when instantiating 1-D set")
                scols = [sname]
    ...
    def _declare_set_with_dict(
        ...
        indexed set with its own indices set as keys. Otherwise, an Ordered Scalar Set will be created, either
        from the keys or the values of 'sdata' depending on the value for 'use_values' (False for keys, True 
        for values).
    ...
    def declare_ordered_time_set(
        self,
        sname: str,
        *sets: pyo.Set
    ):
        """Unnest the time sets into a single, unnested ordered, synchronous time set, an IndexedSet object keyed by the values in the time set, 
        and an IndexedSet object keyed by the combined, original input sets.
        ...
        In summary, this function creates three sets, creating a unique, ordered set from input sets with the assumption that they are given 
        to the function in hierarchical order. For example, for a desired time set that orders Year, Month, Day values, the *args for the function
        ...
        *sets : pyo.Set
        ...
        return_set : bool | None, optional
            Return the set rather than assign within function, by default False
        create_indexed_sets: bool | None, optional
            Create two sets that walk between 
    ...
    def declare_shifted_time_set(
        ...
        *sets: pyo.Set,
        ...
        For example, with a storage constraint where the current value is contrained to be equal to the value 
        of storage in the previous period:

        model.storage[t] == model.storage[t-1] + ...

        The indexing set must be consistent with the storage variable, but not include elements that are undefined for 
        this constraint. In this example, the set containing values for t must not include t = 1 (e.g. the lagged value must 
        be defined). This function creates a shifted time set by removing values from the input sets to comply with the 
        lags or leads.

        Function inputs require a shift size (in the example above, this would be 1), a shift type (lead or lag), and the sets
        used to construct the new, shifted set (model.timestep). If a lag or lead is required on a single dimension of the new set, 
        the 'shift_sets' argument can include a list of pyo.set names (included in the arguments) to shift by the other args.

        For example...

        model.storage[hub, season] == model.storage[hub, season - 1]

        In this case, season = 1 is always invalid due to the lag; so index (1, 2) or the value for hub = 1 and season = 2 is valid, 
        but (2, 1) remains an invalid argument as there is no season = 0. A new set composed of hub and season, with shift_sets = ["season"]
        and *sets = model.hub, model.season, is created to lag on one index value while leaving others unchanged. 
        ...
        *sets: Unnamed arguments
        ...
        """
        # declare_ordered_time_set  -- Unpack sets and assign indices to column dictionary; no need to return set product
        self.unpack_set_arguments(sname, sets, return_set_product = False)

        # declare_lagged_time_set  -- if time_sets provided, use timesets to lag; unpack into list to modify set objects
        ...
        for shift_set in shift_sets:
            if shift_set not in setnames:
                raise ValueError(f"You provided a shift set that was not included in the set arguments: {shift_set}")
    ...
    def declare_param(
        ...
        Unpacks column dictionary of index set for param instance and creates pyo.Param; either assigns the 
        value internally or returns the object based on return_param.
    ...
    def declare_var(
        ...
        within: Literal["NonNegativeReals", "Binary", "Reals", "NonNegativeIntegers"] | None = "NonNegativeReals",
    ...
    def unpack_set_arguments(
        ...
        sname: str,
        sets: Tuple[pyo.Set],
        return_set_product: bool | None = True
    ) -> pyo.Set:
        """Handles unnamed pyo.Set arguments for multiple declaration functions.

        For an arbitrarily large number of set inputs, this function unpacks the names for each set stored in the
        column dictionary, creates a new list of the index labels and ordering, and then provides the pyo.Set product
        result as an output.
        ...
        if not did_you_provide_any_sets:
            raise ValueError("No sets provided in *args; provide pyo.Set objects to use this function")
    ...
    def get_duals(
        ...
        if not isinstance(component, pyo.Constraint):
            raise ValueError(f"Component {component} is not a pyo.Constraint object; cannot extract duals")
    ...
    class DefaultDecorator:
        ...
        Upon initialization, the decorator handles model assignment at class level to ensure inheriting
        classes have access to the models within local scope.
        """
    ...
    class ParameterExpression(DefaultDecorator):
        ...
        def __init__(self, *args, **kwargs):
            """Upon initialization, assign the model instance stored in DefaultDecorator to scope of constraint 
            expression. Named and unnamed keywords assigned to instance.

            Arguments must be pyomo set instances, and named keywords need to be additional parameters passed to 
            pyo.Constraint. This works in practice the same as decorator use in Pyomo.
        ...
        def __call__(self, expression):
            """Upon instantiation, __call__ unpacks indices for each argument in decorator and assigns to 
            cols_dict. Keyword arguments unpack into creation of parameter object, and then parameter is 
            assigned to the model instance pointed to in DefaultDecorator
    ...
    class ConstraintExpression(DefaultDecorator):
        ...
        def __call__(self, expression):
            """Upon instantiation, __call__ unpacks indices for each argument in decorator and assigns to 
            cols_dict. Keyword arguments unpack into creation of constraint object, and then constraint is 
            assigned to the model instance pointed to in DefaultDecorator
