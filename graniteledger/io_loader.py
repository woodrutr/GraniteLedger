"""Utility helpers for loading the ``graniteledger.io.frames_api`` module safely."""
...
def _load_frames_module():
    module_name = 'graniteledger.io.frames_api'
...
try:
    ...
    def _raise_pandas_error() -> None:
        raise ImportError(
            "pandas is required for graniteledger.io.frames_api; install it with `pip install pandas`."
        ) from original_exc
