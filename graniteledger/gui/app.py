...
try:  # pragma: no cover - optional dependency
    from graniteledger.engine.run_loop import run_end_to_end_from_frames as _RUN_END_TO_END
except ModuleNotFoundError:  # pragma: no cover - optional dependency
    _RUN_END_TO_END = None

try:  # pragma: no cover - optional dependency
    from graniteledger.io_loader import Frames as _FRAMES_CLASS
except ModuleNotFoundError:  # pragma: no cover - optional dependency
    _FRAMES_CLASS = None

if TYPE_CHECKING:  # pragma: no cover - type-checking only
    from graniteledger.io_loader import Frames as FramesType
    import pandas as pd
else:
    FramesType = Any
...
DEFAULT_CONFIG_PATH = Path(PROJECT_ROOT, 'graniteledger', 'src', 'common', 'run_config.toml')
...
def _build_policy_frame(
    ...
    *,
    ...
    """Construct the policy frame consumed by :class:`graniteledger.io_loader.Frames`."""
...
def run_policy_simulation(
    ...
    *,
    ...
    ``(stage, payload)`` convention emitted by
    :func:`graniteledger.engine.run_loop.run_end_to_end_from_frames`.
    """
