...
from graniteledger.src.common.utilities import scale_load, scale_load_with_enduses
...
def output_inputs(OUTPUT_ROOT):
    ...
    years = list(
        pd.read_csv(PROJECT_ROOT / 'graniteledger/src/integrator/input/sw_year.csv')
        .dropna()['year']
    )
    regions = list(
        pd.read_csv(PROJECT_ROOT / 'graniteledger/src/integrator/input/sw_reg.csv')
        .dropna()['region']
    )
