...
In main(), we construct the new database, download the raw NOAA data, and then clean/impute data and upload these
to the database
"""
...
def check_cnty_exists(i_fips_cnty, i_year, session, i_dbloc=l_dbloc, i_delobs=l_delobs):
    ...
    """
    cnty_check = (
        session.query(county_noaa)
        .filter(and_(county_noaa.year == i_year, county_noaa.FIPS_cnty == i_fips_cnty))
        .first()
    )
...
def create_county_series(i_fips_cnty, i_year, i_engine_loc=l_dbloc_full):
    ...
    stations = (
        session_func.query(county_isd)
        .filter(and_(county_isd.FIPS_cnty == i_fips_cnty, county_isd.Year == i_year))
        .all()
    )
    ...
    if not data_exist:
        ...
        try:
            ...
            def clean_to_df_dict(
                i_station_id, l_dict=d_clean_vars, i_d_df_stations_raw=d_df_stations_raw
            ):
                """clean_to_df_dict
            ...
            def clean_full_stations(
                l_df,
                i_df_station_id=df_station_id,
                i_d_varlist=d_varlist,
                year=i_year,
                fips_cnty=i_fips_cnty,
            ):
                """clean_full_stations
