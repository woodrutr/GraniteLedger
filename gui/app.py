...
def _build_default_frames(years: Iterable[int]) -> Frames:
    demand_records = [
    ...
    return Frames(base_frames)


def _ensure_years_in_demand(frames: Frames, years: Iterable[int]) -> Frames:
    if not years:
        return frames
...
def run_policy_simulation(
    ...
    *,
    ...
    try:
        frames_obj = Frames.coerce(frames) if frames is not None else _build_default_frames(years)
        ...
        outputs = run_end_to_end_from_frames(frames_obj, years=years, price_initial=0.0)
